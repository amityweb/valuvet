...
Drupal.behaviors.plupload = {
  attach: function (context, settings) {
    $(".plupload-element", context).once('plupload-init', function () {
      ...
      // !!! HACK !!!
      // Limit plupload queue to 8 elements (true for the whole site)
      // ADDED BY <paolo@gnstudio.com>
      $this.pluploadQueue().bind('FilesAdded', function(up, files) {
        var max = 8;
        if (files.length > max) {
          var cutaway = files.length - max;
          alert('You are allowed to add only 8 files.');
          up.splice(max, cutaway); // trunk the queue to "max"
        }
      });
      ...
      if (0 < $(this).find('.plupload-element').length) {
        ...
        $(this).submit(function(e) {
          ...
          $(this).find('.plupload-element').each( function(index){
            ...
            pluploadSettings.required = false;
