...
class TF_DBC { // dynamic database container
     ...
     *
     * !Important: do not change the order of the input array keys and values in different places in the project
     * * This will come to wrong/different columns allocations
     * * Use the same mapping array for specific data placed in container
     * * If you want to add more columns/fields, there are only one safe mode to do that:
     * * * append that columns to the end of the mappingArray (not prepend or place in the middle somewhere)
     * * If you do not do that, all previous data placed in container for specified _group will be wrong/deprecated
     * * * relatively new data and reads/updates
     * * For example you have initial array [a:'varchar', b:'int']
     * * Then you want do add more columns to map (or remove)
     * * The only correct way to do that is to append [a:'varchar', b:'int', c:'varchar']
     * * It is not recommended to remove columns, because if in the future you can want to add again some,
     * * * and new added will contain data from the remove old columns
     * * For the cases when want to remove, correct will be to copy all data 'into' another _group without removed columns
     * * * and use that _group instead
     */
