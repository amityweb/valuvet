...
class TF_SEEK extends TF_TFUSE {
    ...
    public function create_seek_property_post_type() {
        register_post_type($this->post_type, array(
            'labels'            => array(
                ...
                'add_new_item'  => __('Add New ' . $this->model->get_option('seek_property_name_singular', 'Seek Post'), 'tfuse'),
    ...
    public function build_form_search_where_sql($form_id, $exclude_parameters = array()){
        ...
        if( !isset($forms_options[$form_id]) ){
            die('Undefined seek form_id: '.esc_attr($form_id));
        }
    ...
    public function get_search_sql($args = array()){
        ...
        return "
            ". (@$args['noFrom'] ? "" : "FROM " . $this->db_table_name . " AS options") .""
        . (@$args['noJoins'] ? "" : "
        ...
        LEFT  JOIN " . $wpdb->prefix . "terms AS taxonomy_terms         ON taxonomy_terms.term_id       = tr.term_taxonomy_id") ."
            ". (@$args['noWhere'] ? "" : "WHERE p.post_status = 'publish' AND p.post_type = '".$this->post_type."' ");
    }
            ...
            if ( count( array_diff(
                array_keys(
                    array_merge(
                        ...
                        array_fill_keys(
                            array_map(
                                create_function('$e','return array_pop(explode("'.($this->post_type).'", $e));'),
                                array_keys($this->get_property_taxonomies())
                            ),
                            '~'
                        )
                    )
    ...
    private function create_index_table($create_indexes = true) {
        ...
        if ($taxonomies = $this->get_property_taxonomies()) {
            ...
            foreach ($taxonomies as $tax_id=>$tax_obj) {
                ...
                $sql_taxonomies[] = "\n". $tid ." LONGTEXT CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,";
            }
    ...
    public function save_post_extra_processing($tfuse_post_options, $post_id) {
        ...
        if(sizeof($_POST['tax_input'])){
            $terms = array();
            foreach($_POST['tax_input'] as $tax_id=>$tax_vals){
                if(sizeof($tax_vals)){
        ...
        if (sizeof($taxonomies)) {
            foreach ($taxonomies as $tax_id=>$tax_vals) {
                $tfuse_post_options[ array_pop(explode($this->post_type, $tax_id)) ] = implode('', array_keys($tax_vals) );
            }
        }
    ...
    public function get_property_taxonomies($onlyVisible = true) {
        static $cache = NULL;
        if ($cache === NULL)
            $cache = get_taxonomies(array('object_type'=>array($this->post_type)), 'objects');

        $result = array();
        if ($cache)
            foreach ($cache as $tax_id=>$tax_object)
                if ($onlyVisible) {
