...
class TF_UPDATER extends TF_TFUSE {
    ...
    function updates_item_menu_page() {
        ...
        $title = !empty($count) ? 'Updates <span class="update-plugins count-' . $count . '"><span class="update-count">' . number_format_i18n($count) . '</span></span>' : 'Updates';
        add_submenu_page('themefuse', 'Updates', $title, 'manage_options', 'tfupdates', array(&$this, 'themefuse_update_page'));
    }
    ...
    function themefuse_update_check() {
        ...
        if (!empty($_GET['action']) && $_GET['action'] == 'checkagain') {
        ...
        if (!empty($themefuse_update->response[TF_THEME_PREFIX]['suspended']))
            return false;

        // If we're already using the latest version, return FALSE
        if (version_compare($this->theme->framework_version, @$themefuse_update->response[TF_THEME_PREFIX]['Framework']['new_version'], '>=') &&
            version_compare($this->theme->mods_version, @$themefuse_update->response[TF_THEME_PREFIX]['ThemeMods']['new_version'], '>=') &&
            version_compare($this->theme->theme_version, @$themefuse_update->response[TF_THEME_PREFIX]['Templates']['new_version'], '>='))
            return false;
    ...
    function themefuse_update_page() {
        if ('tf-do-upgrade' == $this->theme->action || 'tf-do-reinstall' == $this->theme->action || (isset($_POST['upgrade']) && $_POST['upgrade'] == __('Proceed', 'tfuse'))) {
            ...
            if (isset($_POST['connection_type']) && $_POST['connection_type'] == 'ftp') {
