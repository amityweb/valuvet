...
class TF_AJAX extends TF_TFUSE {
    ...
    public function catch_child_options_submit($arguments) {
        ...
        if (!count($options))
            die('Invalid options given in '.__METHOD__.'(). Only options that sure creates _POST values are allowed (and be sure they are unique enough, a few options in id containing "general"/"not unique" names its not a good idea)');
