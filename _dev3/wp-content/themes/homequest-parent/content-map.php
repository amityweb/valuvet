...
elseif (is_front_page())
{
    ...
    if(tfuse_options('use_page_options') && tfuse_options('homepage_category')=='page'){
        $tmp_conf['post_id'] = $page_id = tfuse_options('home_page');
        $tmp_conf ['show_all_markers'] = false;
...
elseif ((is_page() || is_single()) && (get_post_type($post->ID))!= TF_SEEK_HELPER::get_post_type())
{
    //if is page
    $tmp_conf['post_id'] = $post->ID;
    $tmp_conf ['show_all_markers'] = false;
...
<div class="maptop">
    ...
    <div class="maptop_pane">
        <div class="container_12">
            <div class="maptop_hidebtn"><?php _e('Hide the Map ','tfuse'); ?><span></span></div>
        </div>
    </div>
            ...
            $(".maptop_hidebtn").click(function(){
                if ($(this).closest(".maptop").hasClass("map_hide")) {
                    $(".maptop_content").stop().animate({height:'309px'},{queue:false, duration:550, easing: 'easeOutQuart', complete: function(){
                        google.maps.event.trigger(seek_map.map, 'resize');
                        if (first_map_show && seek_map.homeMarker !== undefined) {
                            seek_map.map.setCenter(seek_map.homeMarker.latLng_);
                        }
                        first_map_show = false;
                    } });
                    $(this).html("<?php _e('Hide the Map ','tfuse'); ?><span></span>");
                } else {
                    first_map_show = false;
                    $(".maptop_content").stop().animate({height:'0px'},{queue:false, duration:550, easing: 'easeOutQuart' });
                    $(this).html("<?php _e('Show the Map ','tfuse'); ?><span></span>");
                }
