 ...
 */
    function tfuse_shortcode_posts($args = null)
    {
        ...
        $tf_cat_ID = ( !empty($category)) ? get_cat_ID( $category ) : '';


        if($sort == 'recent')
        {
            $query = new WP_Query( array ( 'post_type' => $post_type, 'orderby' => 'post_date',  'order '=>'DESC', 'cat'=> $tf_cat_ID, 'posts_per_page'=>$items ) );
