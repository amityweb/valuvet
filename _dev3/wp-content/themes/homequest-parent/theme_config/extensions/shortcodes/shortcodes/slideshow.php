 ...
 */
    ...
    function tfuse_slideshow($atts, $content = null)
    {
        ...
        extract(shortcode_atts(array('width' => 400, 'height' => 300, 'play' => 4000), $atts));

        $content = trim($content);
        $image_arr = (array) preg_split("/(\r?\n)/", $content);
        $uniq = rand(300, 400);
/*        $realheight = $height-(count($image_arr)/2-1);*/
        $out = '<script type="text/javascript">
        jQuery(document).ready(function(){
        jQuery(".slides_container").css("height","'. $height .'")
            jQuery("#slides' . $uniq . '").slides({
                play: ' . $play . ',
                hoverPause: true,
                autoHeight: true,
                effect: "fade",
        ...
        $out .= '<div id="slides' . $uniq . '" class="slideshow slideGallery"><div class="slides_container">';
