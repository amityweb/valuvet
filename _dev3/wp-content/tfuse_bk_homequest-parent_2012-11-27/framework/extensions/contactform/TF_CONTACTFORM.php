...
class TF_CONTACTFORM extends TF_TFUSE
{
    ...
    function __init()
    {
        if(!file_exists(TEMPLATEPATH.'/theme_config/extensions/contactform/')) return;
    ...
    function add_static()
    {
        ...
        $this->include->css('contact_form', 'tc_css', 'tf_head');
        ...
        $this->include->js('contact_form', 'ext_contactform_js', 'tf_footer');
    }
    function add_menu()
    {
    ...
    function save_form()
    {
        if(wp_verify_nonce($_POST['form_setup_nonce'],'form_setup_nonce_action'))
        {
            ...
            foreach($_POST['tf_cf_select'] as $key => $value) {
                ...
                if(isset($_POST['tf_cf_input_newline_' . $key]))
                    $form_to_save['input'][$key]['newline'] = 1;

                if(isset($_POST['tf_cf_input_required_' . $key]))
