...
class TF_SHORTCODES extends TF_TFUSE {
    ...
    function add_static() {
        ...
        $this->include->css('shortcodes', 'ext_shortcodes_css');
        if (is_admin()) {
            $this->include->js('shortcodes', 'ext_shortcodes_js', 'tf_footer');
            $this->include->js('shortcodes_custom_generators', 'ext_config_shortcodes_js', 'tf_footer');
            $this->include->css('lionbars', 'framework_css');
    ...
    function load_shortcode_files($force = FALSE) {
        if (!is_admin() || $force === TRUE) {
            foreach (glob(TFUSE_EXT_CONFIG . '/' . strtolower($this->_the_class_name) . '/shortcodes/*.php') as $filename) {
                locate_template(TFUSE_EXT_CONFIG_DIR . '/' . strtolower($this->_the_class_name) . '/shortcodes/' . basename($filename), TRUE);
            }
        }
    }
    ...
    function tfuse_ajax_add_shortcode_page() {
        ...
        $categories = array('all' => 'Show all');
        foreach ($flipped as $key => $val)
            $categories[$key] = $val;
        $data['category_option'] = array(
            'name' => 'Shortcode Categories',
            'desc' => 'Shortcode Categories',
        ...
        $data['filter_option'] = array(
            'name' => 'Shortcode Text Fitler',
            'desc' => 'Shortcode Text Filter',
        ...
        tfjecho(array('status' => 1, 'content' => $out, 'shc_ids' => $shc_ids));
    }
