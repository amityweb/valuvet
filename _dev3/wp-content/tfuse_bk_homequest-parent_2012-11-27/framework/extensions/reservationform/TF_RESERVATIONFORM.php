...
class TF_RESERVATIONFORM extends TF_TFUSE
{
    ...
    function __init()
    {
        if(!file_exists(TEMPLATEPATH.'/theme_config/extensions/reservationform/')) return;
    ...
    function add_static()
    {
        $this->include->register_type('tc_css', TEMPLATEPATH . '/theme_config/extensions/'. strtolower($this->_the_class_name) .'/static/css');
        ...
        $this->include->css('reservation_form', 'tc_css', 'tf_head');
        $this->include->register_type('ui.selectmenu', TEMPLATEPATH . '/js');
        $this->include->js( 'ui.selectmenu' , 'ui.selectmenu' ,'tf_footer' , 20);
        ...
        $this->include->js('reservation_form', 'ext_reservationform_js', 'tf_footer');
    }

    function add_menu()
    {
        add_object_page('Reservation Forms Settings', "Reservations", 'publish_pages', 'tf_reservations_list', array($this, 'list_reservations'));
        add_submenu_page('tf_reservations_list', 'User Reservations', 'User Reservations', 'publish_pages', 'tf_reservations_list', array($this, 'list_reservations'));
        add_submenu_page('tf_reservations_list', 'All Reservation Forms', 'All Reservation Forms', 'publish_pages', 'tf_reservation_forms_list', array($this, 'list_forms'));
        add_submenu_page('tf_reservations_list', 'Add New Form', 'Add New Form', 'publish_pages', 'tf_reservation_form', array($this, 'show_add_form'));
        add_submenu_page('tf_reservations_list', 'General Settings', 'General Settings', 'publish_pages', 'tf_reservation_forms_gensett', array($this, 'list_gen_options'));
      }
    ...
    function list_reservations()
    {
        ...
        if(isset($_GET['s'])){
            ...
            $posts[] = wp_get_single_post($post_id);
        }
        else
    ...
    function add_email_content(){
        ...
        $resId = $_GET['id'];
        $status = $_GET['status'];
        $the_post = wp_get_single_post($resId);
    ...
    function reservation_details()
    {
        ...
        foreach ($the_form['input'] as $key=>$input) {
            ...
            $option = array();
            $option['name'] = urldecode($input['label']);
            $option['desc'] = '';
    ...
    function ajax_send_email($ph_id = -1){
        ...
        $message = ($status == 1) ? urldecode($the_form['confirm_email_template'] ):urldecode($the_form['reject_email_template']);
        if(trim($message) != ''){
            foreach($the_form['input'] as $input){
    ...
    function submitFrontendForm()
    {
        ...
        $message = '';
        $title = (trim(urldecode($form['reservation_title'])) != '') ? urldecode($form['reservation_title']) : 'New Reservation';
        $post_content = array();
            ...
            if(isset($form['new_res_email_template']) && trim($form['new_res_email_template']) != '')
            {
                ...
                $message = str_replace('[resnumber]',$nr,$message);
                        foreach($form['input'] as $input){
                            if($_POST[TF_THEME_PREFIX.'_'.$input["shortcode"]])
        ...
        echo json_encode(array('mess'=>urldecode($form['succes_mess']),'error' => false));
        } else echo json_encode(array('mess'=>urldecode($form['fail_mess']),'error' => true));
        die;
    }
    ...
    function save_form()
    {
        ...
        foreach ($_POST['tf_rf_select'] as $key => $value) {
            ...
            if (isset($_POST['tf_rf_input_newline_' . $key])) {
                $form_to_save['input'][$key]['newline'] = 1;
            }
            if (isset($_POST['tf_rf_input_required_' . $key])) {
        ...
        $form_to_save['reset_button'] = $_POST['tf_rf_mess_reset'];
        ...
        foreach ($options['tabs'] as $tab) {
            ...
            foreach ($headings as $heading) {
                ...
                if ($id !== false) {
                    ...
                    foreach ($heading['options'] as $option) {
                        if ($heading['id'] == 'form_settings' || $tab['id'] == 'tf_rf_dates_settings') continue;
    ...
    {
        if (is_null(wp_get_single_post($_id)))
