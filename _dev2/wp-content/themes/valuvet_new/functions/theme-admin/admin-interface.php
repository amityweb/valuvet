...
$functions_path = VV_FILEPATH . '/functions/theme-admin/';
...
function optionsframework_add_admin() {
    ...
    if ( isset($_REQUEST['page']) && $_REQUEST['page'] == 'optionsframework' ) {
		if (isset($_REQUEST['of_save']) && 'reset' == $_REQUEST['of_save']) {
			$options =  get_option('vv_template'); 
    ...
    $of_page = add_object_page('My Theme Options', 'Theme Options', 'edit_theme_options', 'optionsframework', 'optionsframework_options_page');
...
function optionsframework_options_page(){
    $options =  get_option('vv_template');      
    $themename =  get_option('vv_themename');
?>
...
<div class="wrap" id="of_container">
  ...
  <form action="" enctype="multipart/form-data" id="ofform">
    ...
    <img style="display:none" src="<?php echo bloginfo('stylesheet_directory'); ?>/functions/theme-admin/images/loading-bottom.gif" class="ajax-loading-img ajax-loading-img-bottom" alt="Working..." />
...
function of_style_only() {
	wp_enqueue_style('admin-style', OF_DIRECTORY.'/functions/theme-admin/admin-style.css');
	wp_enqueue_style('color-picker', OF_DIRECTORY.'/functions/theme-admin/css/colorpicker.css');
}	
...
function of_load_only() {
	...
	wp_register_script('jquery-input-mask', OF_DIRECTORY.'/functions/theme-admin/js/jquery.maskedinput-1.2.2.js', array( 'jquery' ));
	wp_enqueue_script('jquery-input-mask');
	wp_enqueue_script('color-picker', OF_DIRECTORY.'/functions/theme-admin/js/colorpicker.js', array('jquery'));
	wp_enqueue_script('ajaxupload', OF_DIRECTORY.'/functions/theme-admin/js/ajaxupload.js', array('jquery'));
...
<script type="text/javascript" language="javascript">
		...
		jQuery(document).ready(function(){
			...
			//Color Picker
			<?php $options = get_option('vv_template');
...
function optionsframework_machine($options) {
	...
	foreach($options as $value) {
		...
		case "typography":
			...
			if (strpos($val, 'Tahoma') !== false){ $font06 = 'selected="selected"'; }
			if (strpos($val, 'Palatino') !== false){ $font07 = 'selected="selected"'; }
			if (strpos($val, 'Helvetica') !== false){ $font08 = 'selected="selected"'; }			
			if (strpos($val, 'Arimo') !== false){ $font09 = 'selected="selected"'; }
			if (strpos($val, 'Cantarell') !== false){ $font10 = 'selected="selected"'; }
			if (strpos($val, 'Droid Sans') !== false){ $font11 = 'selected="selected"'; }
			if (strpos($val, 'Nobile') !== false){ $font12 = 'selected="selected"'; }
			if (strpos($val, 'PT Sans') !== false){ $font13 = 'selected="selected"'; }
			
			$output .= '<select class="of-typography of-typography-face" name="'. $value['id'].'_face" id="'. $value['id'].'_face">';
			$output .= '<option value="Arimo" '. $font09 .'>Arimo</option>';
			$output .= '<option value="Cantarell" '. $font10 .'>Cantarell</option>';
			$output .= '<option value="&quot;Droid Sans&quot;" '. $font11 .'>Droid Sans</option>';
			$output .= '<option value="Nobile" '. $font12 .'>Nobile</option>';
			$output .= '<option value="&quot;PT Sans&quot;" '. $font13 .'>PT Sans</option>';			
			$output .= '<option value="Arial" '. $font01 .'>Arial</option>';
			$output .= '<option value="Verdana" '. $font02 .'>Verdana</option>';
			$output .= '<option value="Trebuchet" '. $font03 .'>Trebuchet</option>';
