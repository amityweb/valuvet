...
class TF_OPTIGEN extends TF_TFUSE {
    ...
    function multi_upload($opts){
        ...
        $button_text = ($num_images) ? 'Add/Edit Images' : 'Upload Images';
        $output .= '<div class="upload_button_div"><span class="attachment_num">'.$num_images.'</span> Images Uploaded<a tab='.$tab.' href="#" class="multi_upload button upload_button '.$post_class.'" id="' . $id . '_button" rel="' . $post_id . '">' . __($button_text) . '</a> </div>';
 ...
 /*   function selectsearch($opts)
    ...
    function selectsearch($opts)
    {
        ...
        if(!$this->include->type_is_registered('chosen_js')){
            ...
            $this->include->register_type('chosen_css', TFUSE . '/static/css');
            $this->include->css('chosen', 'chosen_css', 'tf_head', '0.9.8');
            $this->include->css('chosen_ext', 'chosen_css', 'tf_head', '0.9.8');
        }
        $class = '';
        $style = '';
        $groups=array();
        ...
        if($show_groups and $multiple){
            ...
            foreach($groups as $group)
                $str .= '<a href="" data-placeholder="'.$group.'">'.ucwords(str_replace('_',' ',$group)).'</a>'."\n";
            ...
            $this->include->js('chosen_group', 'chosen_js');
    ...
    function maps($opts) {
        ...
        if(!$this->include->type_is_registered('optigen_maps_js')){
            ...
            $this->include->js('googleMapsOptigen', 'optigen_maps_js', 'tf_footer', 10, '1.2');
        }
