...
class TF_GET_IMAGE {
    ...
    function get_img() {
        $this->resize();
        $this->out = $this->before . '<img ' . ($this->width != NULL &&  !$this->removeSizeParams ? ' width="' . $this->width . '" ' : '') . ' ' . ($this->height != NULL  &&  !$this->removeSizeParams ? ' height="' . $this->height . '" ' : '') . ' src="' . $this->src . '" ' . $this->property_string . ' />' . $this->after;
        return $this->out;
    }
    ...
    function __get($name) {
        if (in_array($name, get_class_methods($this)))
            ...
            echo 'Warning. Property is required: ' . $name;
        }
    }
