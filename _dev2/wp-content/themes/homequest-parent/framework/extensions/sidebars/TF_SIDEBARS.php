...
class TF_SIDEBARS extends TF_TFUSE {
    ...
    function __init()
    {
        ...
        if (stripos($_SERVER['REQUEST_URI'], 'widgets.php')) {
            ...
            if (isset($_GET['tab_switch']))
                $this->include->js_enq('tab_switch', $_GET['tab_switch']);
...
//            }
    ...
    protected function add_static() {
        ...
        $this->include->css('sidebar', 'ext_sidebar_css');
        $this->include->js('sidebar', 'ext_sidebar_js', 'tf_footer', 10, '1.1');
    }
    ...
    public function tfuse_ajax_sidebar_save_new() {
        tf_can_ajax();
        if (trim($_POST['sidebar_name']) == '')
            die(json_encode(array('status' => -1, 'message' => 'No sidebar name specified.')));
        $this->model->save_new_sidebar($_POST['sidebar_name']);
    ...
    public function tfuse_ajax_sidebar_delete() {
        tf_can_ajax();
        $this->model->delete_sidebar($_POST['sidebar_id']);
        echo json_encode(array('status' => 1));
        die();
    }
    ...
    public function tfuse_ajax_sidebars_save() {
        ...
        if (isset($data['last_ids']) && $data['last_ids'] != '') {
            $this->model->delete_settings(array('subtype' => $_POST['type'], 'id' => $data['last_ids']));
        }
        $this->model->save_sidebar_settings($data, $_POST['type']);
        die();
    }
    ...
    public function tfuse_ajax_sidebars_get() {
        ...
        if (strpos($type, 'default_') !== FALSE) {
            ...
            $out = isset($settings[$type][$_POST['data']['id']]) ? $settings[$type][implode(',', sort_sdb(explode(',', $_POST['data']['id'])))] : array();
        }
        tfjecho($out);
        die();
    }
