...
<div id="posts-filter">
    ...
    $filter_type = (isset($_GET['filter'])) ? $_GET['filter'] : 1;
    ...
    $paged = (isset($_GET['paged']) && $_GET['paged']>0 && $_GET['paged']<=$nr_of_pages)?$_GET['paged']:1;
    $is_filter = (isset($_GET['filter'])) ? $_GET['filter'] : false;
...
<table cellspacing="0" class="wp-list-table widefat fixed pages form_list_table">
    ...
    if (isset($posts[0]->ID) > 0)
        foreach ($posts as $post) {
            $form =isset($post->term[0]->description)?unserialize($post->term[0]->description):unserialize($post->term[0]);
