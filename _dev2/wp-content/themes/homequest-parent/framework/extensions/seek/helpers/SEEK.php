...
class TF_SEEK_HELPER {
    ...
    public static function print_all_not_form_hidden($form_id, $exclude_params = array()){
        ...
        foreach($forms_ids as $form_id){
            ...
            if( !isset( $forms[$form_id] ) ){
                echo 'Undefined seek form_id: '.esc_attr($form_id).", in ::print_all_not_form_hidden()<br/>\n";
                return;
            }
            ...
            if(sizeof($form['items'])){
                foreach($form['items'] as $item_id){
                    ...
                    if( !isset( $items_options[$item_id] ) ){
                        echo 'Udefined form item id: '.esc_attr($item_id).", in ::print_all_not_form_hidden()<br/>\n";
                        return;
                    }
    ...
    public static function get_option($id, $default = NULL){
        global $TFUSE;
        return $TFUSE->ext->seek->get_seek_option($id, $default);
    }
    ...
    public static function get_items_options(){
        global $TFUSE;
        return $TFUSE->ext->seek->get_items_options();
    }

    // Get forms options array from theme_config
    public static function get_forms_options(){
        global $TFUSE;
        return $TFUSE->ext->seek->get_forms_options();
    }
    ...
    public static function gen_search_link($form_id, $parameters_values = array(), $only_parameters = false){
        ...
        if(!isset($forms_options[ $form_id ])){
            die('Seek search link generator error: Ivalid form_id - '.esc_attr($form_id));
            return;
        }
    ...
    public static function get_form_item_param_name($item_id, $form_id = NULL){
        ...
        if(!isset($itemsOptions[$item_id])) die('Error: Cannot get form item parameter name. No item with id='.esc_attr($item_id));

        if($form_id !== NULL){
            $formsOptions = self::get_forms_options();

            if(!isset($formsOptions[$form_id])) die('Error: Undefined form_id='.esc_attr($form_id));

            if(!in_array($item_id, $formsOptions[$form_id]['items'])) die('Error: Item_id='.esc_attr($item_id)).' is not in form_id='.esc_attr($form_id).' items';
        }
