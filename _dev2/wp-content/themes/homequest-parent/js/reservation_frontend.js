...
jQuery(document).ready(function(){
    ...
    $('.tfuse_rf_post_datepicker_in').datepicker({
        ...
        {
            inst.dpDiv.css({marginTop: -input.offsetHeight+38 + 'px'});
        }
...
function showErrorMessage(textMessage){
    _message= jQuery('#form_messages').html('<h2>'+textMessage+'</h2>').addClass('error_submiting_form').show();
    _form.hide();
}
function day_statuses(date){
    if(jQuery(this).hasClass('tfuse_rf_post_datepicker_out') && picker_in !==false){
        date_from = picker_in.split('-');
        date_from = new Date(date_from[0], date_from[1] - 1, date_from[2]);
        if(date < date_from) return [false]
    } else if(jQuery(this).hasClass('tfuse_rf_post_datepicker_in') && picker_out !==false){
...
function showSuccessMessage(textMessage){
    _message= jQuery('#form_messages').html('<h2>'+textMessage+'</h2>').addClass('success_submited_form').show();
    _form.hide();
}
function check_fields() {
    ...
    if( _captcha_resp && _return_val )
    {
        ...
        jQuery('#header_message, hr').hide();
        _form.find('#sending').css('display','inline-block');
    }
