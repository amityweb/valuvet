...
function tf_reservationform_shortcode($atts){
    ...
    if($form_exists){
        ...
        $out.='<div class="add-comment-title"><h3 id="header_message">'.__(urldecode($form['header_message']),'tfuse').'</h3></div>';
        ...
        foreach($form['input'] as $form_input){
            ...
            if($is_preview && $input['type'] == 'select' && $browser[0]!='safari') $form_input['ewidthpx'] -=22;
        ...
        $out.='<input type="submit" id="send_reservation" class="btn-send btn-submit" name="submit" title="Submit mesage" value="'. __(urldecode($form['submit_mess']),'tfuse').'" />
        </p></div></form></div></div>';
    } else {
