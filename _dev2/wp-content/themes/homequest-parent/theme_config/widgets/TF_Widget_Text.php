...
class TF_Widget_Text extends WP_Widget {
    ...
    function update( $new_instance, $old_instance ) {
        ...
        $instance['rem_div'] = isset($new_instance['rem_div']);

        return $instance;
    }

    function form( $instance ) {
        $instance = wp_parse_args( (array) $instance, array( 'title' => '', 'class' => '', 'text' => '', 'nopadding' => '', 'rem_div' => '' ) );
    ...
    <p><label for="<?php echo $this->get_field_id('class'); ?>"><?php _e('Class:','tfuse'); ?></label>
    ...
    <p><input id="<?php echo $this->get_field_id('apply_filters'); ?>" name="<?php echo $this->get_field_name('apply_filters'); ?>" type="checkbox" <?php checked(isset($instance['apply_filters']) ? $instance['apply_filters'] : 0); ?> />&nbsp;<label for="<?php echo $this->get_field_id('apply_filters'); ?>"><?php _e('Disable filters','tfuse'); ?></label></p>
    <p><input id="<?php echo $this->get_field_id('rem_div'); ?>" name="<?php echo $this->get_field_name('rem_div'); ?>" type="checkbox" <?php checked(isset($instance['rem_div']) ? $instance['rem_div'] : 0); ?> />&nbsp;<label for="<?php echo $this->get_field_id('rem_div'); ?>"><?php _e('Remove text div','tfuse'); ?></label></p>
