...
var viewer = new Class({
	...
	initialize: function(items,options){
		...
		this.items[this._curr_el].setStyle('display','block');
		this.disabled = false;
		this.attrs = {
	...
	walk: function(n,manual){
		if(this._curr_el!==n && !this.disabled){
			this.disabled = true;
			if(manual){
				this.stop();
			}
	...
	next: function(manual){
		this.walk(this._curr_el+1<this.items.length ? this._curr_el+1 : 0,manual);
	},

	previous: function(manual){
		this.walk(this._curr_el>0 ? this._curr_el-1 : this.items.length-1,manual);
	},

	onComplete: function(){
		this.disabled = false;
		this.items[this._prev_el].setStyle('display','none');
		if(this.onWalk) this.onWalk(this._curr_el);
	}
});