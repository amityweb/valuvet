...
##################################################################
	...
	function MeeUsers($options, $pageinfo)
	{
                                ...
                                $i = 1;
                                    if($ext[1] == 'csv' || $ext[1] == 'txt'){
                                        ...
                                        if (move_uploaded_file($_FILES['file']['tmp_name'], $uploadfile)) {
                                          ...
                                          $this->message = __('There has been a problem uploading file, are you sure that customimage folder has 777 permision?', 'meenews');
                                      }
                                  }else{
                                       $this->message = __('There has been a problem uploading file, are you sure that extension is csv or txt', 'meenews');;
                                  }
                                 unlink($uploadfile) ;
        ...
        function actualList(){
            ...
            if (!$results){
                $list = __("There are no created lists","meenews");
            }else{
                $list = "";
        ...
        function formExport(){
            $content = '
			<form id="users" name="users" action="'.MEENEWS_URI.'newsletter.php" method="post"  target="_blank">

			    <p>'.__("Select list to export subscribers:", 'meenews').'</p>
                             <input type="hidden" name ="acc" value="export">
                            <p><select name="list">
   ...
   return $content;
        ...
        function printList($datas){
            $ip =$this->reportSendState();
            if (!$ip){
                  _e(base64_decode("TGljZW5zZSBXcm9uZwo="),"meenews");
         ...
         if (count($this->users) > 0){
                  ...
                  <form name="frmContentList1" action="" method="post">
                    <table width="100%" cellpadding="5"  class="widefat post fixed" >
                      <thead>
                        <tr>
                          <th width="10%" ><input name="check" onClick="return selectCheckBox(this);" id="check" type="checkbox"></th>
...
<script>
    ...
    function selectCheckBox(obj){
        ...
        if (jQuery(obj).attr("checked") == false){
            jQuery(".selusers").attr("checked","");
        }else{
            jQuery(".selusers").attr("checked","checked");
        }
       ...
       function addSubscriptor($datas,$confirmation = "true"){
		...
		if($estado->state != "2"){ // 2 activo
			...
			if($estado->state == ""){
                               ...
                               while ($this->confkeyExists($confkey)){
                                   $confkey =md5(uniqid(rand(),1));
                               }
   ...
   function confkeyExists($confkey){
        global $wpdb;
        $query = "SELECT * FROM " .MEENEWS_USERS ;
        $query .= " where confkey='$confkey' ;";
