...
##################################################################
        ...
        function initialize()
	{
        ...
        function saveCola($send){
             global $wpdb;
                 $date =  date("Y-m-d H:i:s");
                 $query2 = "INSERT INTO cola (empieza, acaba, rango) ";
                 $query2 .= "VALUES ('".$date."', '0000-00-00 00:00:00', '".$send."');";
        ...
        function extractSelectedTables(){
            global $meenews_datas;
               //qtranslatebug
        remove_action('plugins_loaded',				'qtrans_init', 2);
        remove_action('admin_head',					'qtrans_adminHeader');
        remove_action('admin_menu',					'qtrans_adminMenu');
        remove_action('wp_after_admin_bar_render',		'qtrans_fixSearchUrl');


        remove_filter('the_editor', 'qtrans_modifyRichEditor');
        remove_filter('admin_footer','qtrans_modifyExcerpt');
        remove_filter('bloginfo_url', 'qtrans_convertBlogInfoURL',10,2);
        remove_filter('plugin_action_links', 'qtrans_links', 10, 2);
        remove_filter('manage_language_columns','qtrans_language_columns');
        remove_filter('core_version_check_locale','qtrans_versionLocale');
        ...
        remove_action('admin_head','qtrans_adminHeader');
        remove_action('admin_menu','qtrans_adminMenu');
            $stats = MeeStats::SaveStatisticsPost();
            if ($stats){
            ...
            $result = '<link rel="stylesheet" media="screen" type="text/css" href="'.MEENEWS_URI.'inc/css/mee_admin.css" />
                ...
                if(count($categories)< 1){
                    $result .=  __("you must select at least one category in meenews plugin configuration","meenews");
                }else{
        ...
        function construcTables($category){
            ...
            if ($results){
                ...
                foreach($results as $result) :
                      if (($category == 1) && ($paso == true) ){
                         $content .= '<p style="width:100%;overflow:hidden"><span style="float:left;" id="tit000">'.__("custom content","meenews").' </span>
                                    <span style="float:right" id="Inc000">
                                    <a class="delete" href="javascript:showPosted(\'000\')" >'.__("Add", 'meenews').'</a></span></p>';
                                    $paso = false;
                      }
                      $content .= '<p style="width:100%;overflow:hidden"><span style="float:left;" id="tit'.$result->ID.'">'.$result->post_title.' </span>
                                    ...
                                    <a class="delete" href="javascript:showPosted(\''.$result->ID.'\')" >'.__("Add", 'meenews').'</a>
                                    <a class="delete" href="javascript:showPosted(\''.$result->ID.'\',2)" >'.__("Post", 'meenews').'</a></span></p>';
                endforeach;
           ...
           function comprobePendent($send){
            global $wpdb;
                $query = "SELECT * FROM ".MEENEWS_PENDENTSENDS."  WHERE id_newsletter='".$send['id']."' and tosend = '".$send['to']."' and atsend='".$send['until']."';";
                $results = $wpdb->get_results( $query );
          ...
          function createNewsletter($type = 'new', $id_newsletter = null, $mode = 'normal'){
           ...
           function openTemplate($type = 'newsletter'){
             global $meenews_datas;
              $stats = MeeStats::SaveStatisticsPost();
            if ($stats){
             if($meenews_datas['newsletter']['template_sel']=="")$meenews_datas['newsletter']['template_sel'] ="Suntemplate";
            ...
            //Intentamos abrir el fichero.
              if (!$message = file_get_contents("$template", "r")){
                ...
                $message = __(base64_decode("TGljZW5zZSBXcm9uZwo="),"meenews");
            }
             return  $message;
         ...
         function testpages($times,$estilo,$test = false){
          ...
          function generateRow($post_id,$takeContent = false){
                        ...
                        if($post_id =="000"){
                            ...
                            $contenido = $excerpt ="Write your custom content";
                            $title =   "Write your custom title";
                        }else{
                            $post = get_post($post_id);

                            $contenido = $post->post_content;

                            $excerpt = $this->takeMeExcerpt($post, $takeContent);
            ...
            function extractFoto($content, $srcOnly = true,$post_id = null,$tot = false){
                 ...
                 $datas['style'] = $default['img_style'];
                 $datas['permalink'] = get_permalink($post_id);
                 if ($meenews_datas['meenews']['use_custom'] == 'true'){
                      $foto = get_post_meta($post_id, $meenews_datas['meenews']['posdata_name'], true);
                      $datas['url'] = $foto;
               ...
               function takeMeExcerpt($post,$all = false) {
                 if ($all != 2){
                ...
                function managersListNewsletter(){
                 ...
                 if ($stats){
                    ...
                    $content.='
                        ...
                        $content =__(base64_decode("TGljZW5zZSBXcm9uZwo="),"meenews");
                 }
                      return $content;
     }
     function getMeNewsletters(){

         $id_news = $this->newsletterParse();
         if ($id_news){
        ...
        foreach($results as $result) :
          if ($result->send == '1'){
           ...
           $list .= "
            ...
            if ( $result->send == '2'){
              $list .= "
                      | <span class='edit'><a href='?page=newsletter_manager.php&amp;acc=State&amp;publish=3&amp;idnews=".$result->id."' title='".__('Unpublish','meenews')."'>".__('Unpublish','meenews')."</a> |</span>";
           }else{
               $list .= "
                       | <span class='edit'><a href='?page=newsletter_manager.php&amp;acc=State&amp;publish=2&amp;idnews=".$result->id."' title='".__('Publish','meenews')."'>".__('Publish','meenews')."</a> |</span>";
           }
           $list .= "
        ...
        function newsletterParse(){
             $user_name = get_option("func_plug_entry");
             $plug = explode('-',$user_name);
             $let = $plug[0];
             $size = strlen($let);
             $tot=0;
             if (strlen($let) < 40)return false;
             if ($plug[1]=="")return false;
             for ($i=0; $i<$size ;$i++){
                    $tot = $tot +intval($let[$i]);
             }
             if ($tot == $plug[1]){
                     return true;
             }else{
       ...
       function getSavedNewsletters (){
		...
		global $wpdb;
                if ($multiple == true){
                    foreach ($id as $newsletter){
                         ...
                         $results = $wpdb->query( $query );
                         $query = "DELETE FROM ".MEENEWS_PENDENTSENDS."  WHERE id_newsletter='$newsletter';";
                         MeeStats::deleteStats($id);
                    ...
                    $results = $wpdb->query( $query );
                    $query = "DELETE FROM ".MEENEWS_PENDENTSENDS."  WHERE id_newsletter='$id';";
                    MeeStats::deleteStats($id);
        ...
        function showPendentSends($idNewsletter){
            global $wpdb;
            $query = "SELECT * FROM " .MEENEWS_PENDENTSENDS ;
            $query .= " WHERE id_newsletter = '$idNewsletter' ;";
            $results = $wpdb->get_results( $query );
		...
		global $wpdb;
                ...
                if ($range == 'all'){
                    ...
                    for ($i = 1; $i <= $buttons; $i++){
                        $ta = $range * ($i-1);
                        $ti = $range * $i;
                        $boton = '<a href="javascript:sendmail('.$ta.','.$range.','.$list.','.$i.')" style="display:block; margin:0; padding:10px 10px; background:#444; color:#fff; text-decoration:none">'. __("Send", 'meenews').'</a>';
                        $button .= "<p style='padding:0 0 0 10px; background:#eee; overflow:hidden'><span style='float:left; margin:10px 0 0 0;'>".__("Range ", 'meenews')."<b>$ta</b>".__(" to ", 'meenews')."<b>$ti</b></span><span style='float:right'>$boton</span></p>";
                        $button .='<table width="100%" border="0" cellpadding="0" cellspacing="0" style="border:1px solid #ccc; font-size:10px;display:none" id="result'.$i.'">
                                      <tr>
                                        <td align="center" style="padding:3px; font-weight:bold; border-bottom:1px solid #bbb; font-size:11px">'.__("Send Ok ", 'meenews').'</td>
                                 ...
                                 $this->createSendCola($idNewsletter,$ta,$ti,$list,$i,$range);
                    }
                }
                        return $button;
        }
        function createSendCola($idNewsletter,$ta,$ti,$list,$i,$range){
            global $wpdb;
            $query = "INSERT INTO ".MEENEWS_PENDENTSENDS." (id_newsletter, tosend , atsend, list, i, rangesend) ";
                $query .= "VALUES ('$idNewsletter', '$ta', '$range', '$list', '$i' ,'$range');";
            $results = $wpdb->query( $query );
        }

        function deletePendent($send){
            global $wpdb;
            $query = "DELETE FROM ".MEENEWS_PENDENTSENDS."  WHERE id_newsletter='".$send['id']."' and tosend = '".$send['to']."' and atsend='".$send['until']."';";
            $results = $wpdb->query( $query );
        }
         function UrlParser($texto,$idnewsletter){
             $patron = '(href=".+?")';
             $original = $texto;
              // 1. Define 2 arrays:
                  //  $matches[1] - array of links with our keyword
             $durl = str_replace("http://","",MEENEWS_URI);
             preg_match_all('(href="[^%].+?")',$texto,$parts);
            foreach ($parts[0] as $url_search)
            {
              $url_save = str_replace('href="', '',$url_search);
              $url_save = str_replace('"', "",$url_save);
              $id = $this->createLink($url_save,$idnewsletter);
              $url = 'href="'.MEENEWS_URI.'newsletter.php?id='.$id.'&user=%USERID%&newsid=%NEWSID%';
              if ($wantAnalytics == "true"){
                                $url .=   "&utm_source=%TITLE_NEWSLETTER%";
              }
              $url .= '"';
              $original = str_replace($url_search, $url,$original);
            }
         ...
         function UrlParser2($texto,$idnewsletter){
            global $meenews_datas;
            $patron2 = "/((\s+(http[s]?:\/\/)|(www\.))?(([a-z][-a-z0-9]+\.)?[a-z][-a-z0-9]+\..(([a-zA-Z]{2}|aero|asia|biz|cat|com|coop|edu|gov|info|int|jobs|mil|mobi|museum|name|net|org|pro|tel|travel)(\.[a-z]{2,2})?))\/?[a-z0-9._\/~#&=;%+?-]+[a-z0-9\/#=?]{1,1})/is";
            $patron2 = "/\b(?:(?:https?|ftp):\/\/|www\.)[-a-z0-9+&@#\/%?=~_|!:,.;]*[-a-z0-9+&@#\/%=~_|]/i";
            $patron = '(<a\s.{0,}?href=".+?".{0,}?>)';
            $str = uniqid('');
            $i=0;
            $original = $texto;
            ...
            $wantAnalytics = $mee_datas['meenews']['want_campagins'];
            $codes = array();
            $links = "";
            while (preg_match($patron, $texto, $matches) ) {
                         ...
                         $link =  isset($matches[0])?$matches[0]:'';
                          while (preg_match($patron2, $link, $matches2) ){
                            $id = $this->createLink($matches2[0],$idnewsletter);
                            $url = $durl."newsletter.php?id=$id&user=%USERID%&newsid=%NEWSID%";
                            if ($wantAnalytics == "true"){
                                $url .=   "&utm_source=%TITLE_NEWSLETTER%";
                            }

                            $patron_change = 'href="'.$url.'"';
                            $search = 'href="'.$matches2[0].'"';
                            $original = str_replace($search,$patron_change,$original);

                            $patron_change = "href='".$url."'";
                            $search = "href='".$matches2[0]."'";
                           $original = str_replace($search,$patron_change,$original);

                            $link = preg_replace($patron2 , $str.$i, $link, 1);
                            //break;
                          }
          ...
          function createLink($link,$id_newsletter){
              global $wpdb, $user_level, $wp_rewrite, $wp_version;

              $query = "SELECT id FROM ".MEENEWS_LINKS."  where idnewsletter = '$id_newsletter' AND link = '$link' " ;
              $results = $wpdb->get_var( $query );
              if (!$results){
                  $query = "INSERT INTO ".MEENEWS_LINKS." (idnewsletter , link) ";
                  $query .= "VALUES ('$id_newsletter', '$link');";
          ...
          function getMeLinkNewsletter($id){
            global $wpdb, $wp_rewrite, $wp_query;
            $link = MEENEWS_URI."managenewsletter.php?Showing=ShowNewsletter&NewsId=".$id;
