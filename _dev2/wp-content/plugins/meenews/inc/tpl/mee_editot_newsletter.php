            ...
            jQuery("#add_post").click(function(){
               ...
               function insertDesign(){
                             jQuery( function($) {
                                ...
                                $("iframe").contents().find("#finalTabla").append(post_sel);
                                var titulo = $("iframe").contents().find(".postadded"+idpost).attr('alt');
                                var permalink = $("iframe").contents().find(".postadded"+idpost).attr('rel');
                                $("iframe").contents().find(".listaissue").append("<li id='issue"+idpost+"'><a style=\"<?php echo $issue_style;?>\" href='"+permalink+"'>"+titulo+"</a></li>");
                                jQuery("#designbox").css({'display':'none'});
                              });
               }
               function QuitPosted(idpost){
                       jQuery( function($) {
                                   $("iframe").contents().find(".postadded"+idpost).remove();
                                   $("iframe").contents().find("#issue"+idpost).remove();
                                   $("#Inc"+idpost).html('<a class="delete" href="javascript:showPosted('+idpost+')" ><?php echo _e("Excp", 'meenews'); ?></a>&nbsp;<a class="delete" href="javascript:showPosted('+idpost+',\'all\')" ><?php echo _e("Post", 'meenews'); ?></a>');
                         });
...
<div id="titlewrap">
	...
	<input type="text" name="news_title" size="30" tabindex="1" value="<?php echo $title; ?>" id="title" autocomplete="off" />
        ...
        <br>
<?php
    $the_editor = apply_filters('the_editor', "<div id='editorcontainer'><textarea rows='5' $class cols='5' name='$id' tabindex='$tab_index' id='$id'>%s</textarea></div>\n");


  if ( current_user_can('edit_posts') ) :
   $content = '<div class="editor_holder">'.the_editor($newsletter, $id = 'content', $prev_id = 'title', $media_buttons = true, $tab_index = 2).'</div>';
 endif;
do_action("mee_editor");
do_action("miacction");
?>
    </div>
