...
class WPP_Core {
  ...
  function save_property( $post_id ) {
	...
	$displayname = ( !empty($current_user->display_name) )?$current_user->display_name:$current_user->user_login;
	$addtodo = array(
		'todo_title'       =>  sprintf( 'Property data update by %s', $displayname ),
		'todo_propertyid'  => $post_id	);
	global $todo_class;
  ...
  function shortcode_property_search($atts = "")  {
    ...
    if($atts['ajax_call']) {
        ...
        if($wpp_search = $_REQUEST['wpp_search']) {
          ...
          if(isset($_REQUEST['wpp_search']['requested_page'])) {
        	//** Everything stays the same except for sort order and page */
        	$wpp_query['starting_row']  =  (($_REQUEST['wpp_search']['requested_page'] - 1) * $wpp_query['per_page']);
		  }
          if(isset($_REQUEST['wpp_search']['requested_page'])) {
        //** Figure out current page */
        	$wpp_query['current_page'] =  $wpp_query['requested_page'];
      ...
      $wpp_query['query']['sort_by'] = (!empty($wpp_query['sort_by']) ) ?$wpp_query['sort_by']:'post_date';
      $wpp_query['query']['sort_order'] = (!empty($wpp_query['sort_order']) ) ?$wpp_query['sort_order']:'DESC';
      ...
      $result['top_pagination'] = wppe_draw_pagination(array('return' => true, 'class' => 'wpp_top_pagination', 'sorter_type' => $wpp_query['sorter_type'], 'hide_count' => $hide_count, 'sort_by_text' => $wpp_query['sort_by_text']));
