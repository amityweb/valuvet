    ...
    if($wp_properties['taxonomies']) {
      foreach($wp_properties['taxonomies'] as $taxonomy => $taxonomy_data) {
        ...
        register_taxonomy( $taxonomy, 'property', array(
          ...
          'capabilities' => array('manage_terms' => 'manage_eqfs_categories')
        ));
      }
    }
  ...
  */
  static function get_properties($args = "", $total = false) {
    ...
    if( $query['sort_by'] ) {
      ...
      $sql_sort_order = ($query['sort_order']) ? strtoupper($query['sort_order']) : 'DESC';
    } else {
...
*/
  ...
  static function google_maps_infobox($post, $args = false) {
	...
	$google_address = get_post_custom_values('google_address', $post->ID );
    ob_start(); ?>
    ...
    <?php }  ?>
    <table cellpadding="0" cellspacing="0" class="wpp_google_maps_infobox_table" style="width:100%;" border="0">
      <tr>
        <?php if($image['link']) { ?>
        <td class="wpp_google_maps_left_col" style=" width: <?php echo $image['width']; ?>px">
          ...
          <div class="wpp_google_maps_attribute_row wpp_google_maps_attribute_row_directions_link">
            <a target="_blank" href="http://maps.google.com/maps?gl=au&daddr=<?php echo addslashes(str_replace(' ','+', $google_address[0] )); ?>"><?php _e('Get Directions','wpp') ?></a>
          </div>
          <?php endif; ?>
        </td>
        <?php } ?>
  ...
  function list_table() {
	...
	if( is_admin() ){
		...
		if( $user_level<5 ){
			$wpp_search['post_author']=$user_ID;
		}
	}
