...
class WPP_UI {
  ...
  function property_type_selector($property) {
    global $wp_properties, $user_level, $post;
	...
	$property_payment_status = $property['payment_status'];
?>
      <tr class="wpp_attribute_row_type wpp_attribute_row">
        <th>Payment Status</th>
        <td>
        <?php if( $user_level>=10 ) {?>
          <?php //* Get property types */ ?>
          <select id="wpp_meta_property_status" name="wpp_data[meta][payment_status]" id="payment_status">
            <option value=""></option>
            <option<?php if($property['payment_status']=='Paid') echo ' selected="selected"';?>>Paid</option>
            <option<?php if($property['payment_status']=='Not paid') echo ' selected="selected"';?>>Not paid</option>
          </select>
        <?php $token = get_post_custom_values( 'PAYPAL_TOKEN' , $post->ID); if( !empty($token) ) echo 'Paypal Token : '. $token[0]; ?>
        <?php } else { 
		echo ( !empty($property['payment_status']) )?$property['payment_status']:'Not paid';
		?>
      <?php } ?>
        </td>
      </tr>
...
<?php
    if(count($wp_properties['property_types']) > 1) { ?>
      <tr class="wpp_attribute_row_type wpp_attribute_row <?php if (is_array($wp_properties['hidden_attributes'][$property['property_type']]) && in_array('type', $wp_properties['hidden_attributes'][$property['property_type']])) echo 'disabled_row;'; ?>">
        ...
        <td>
        <?php if( $user_level>=10 || empty($property['property_type']) ) {?>
          <?php //* Get property types */ ?>
          ...
          <?php } ?>
        <?php } else { 
		echo $property['property_type_label'];
		?>
