...
jQuery(document).ready(function ($) {
    ...
    $(".tf_checkbox_switch").unbind('click');
    ...
    $("#tf_rf_add_new").click(function () {
        ...
        count_el = _last_li.find('.single_checkbox:last');
    ...
    $('#tf_rf_form_setup').submit(function () {
        ...
        _inputs.each(function () {
            if (!$(this).closest('ul').hasClass('sortable_options') && $(this).attr('id') != 'tf_rf_email_from' &&
                $(this).closest('.tf_custom_reservationform_dates_row').length == 0
                && $(this).closest('.tf_custom_reservationform_row').find('.'+theme_index+'_inp_select:last option:selected').val() != 6) {
                if ($.trim($(this).val()) == '') {
            ...
            } else if ($(this).closest('ul').hasClass('sortable_options')) {
                _parent_select = $(this).closest('li .tf_custom_reservationform_row').find('.'+theme_index+'_inp_select');
                if (_parent_select.val() == 2 || _parent_select.val() == 4) {
                    if ($.trim($(this).val()) == '') {
                        $(this).css('cssText', 'border-color:red !important');
                        _form_ok = false;
                    } else {
        ...
        if (_form_ok == true) {
            ...
            if (isset_id) {
                ...
                _shortcodes.each(function () {
                    _val = $(this).html();
                    $(this).append("<input type='text' class='hidden' name='tf_rf_input_shortcode[]' value='" + _val.substring(1, _val.length - 1) + "'/>");
                });
            }
...
/*  if($('.tfuse_datepicker').length > 0){
      $('.tfuse_datepicker').removeAttr('id');
    $('.tfuse_datepicker').datepicker({
        dateFormat:"yy-mm-dd",
        minDate: new Date(),
        beforeShowDay:function (date) {
            orientation_from = ($(this).hasClass('tf_exclude_from_datepicker')) ? true : false;
            if (orientation_from) {
                date_to = $(this).parent().find('.tf_exclude_to_datepicker:first').val();
                if (date_to == '')
                    return [true, ''];
                dates = date_to.split('-');
                date_to = new Date(dates[0], dates[1] - 1, dates[2]);
                if (date < date_to)
                    return [true, '']
                else
                    return [false, '']
                ...
                if (date_from == '')
                    return [true, '']
                dates = date_from.split('-');
                date_from = new Date(dates[0], dates[1] - 1, dates[2]);
                if (date > date_from)
                    return [true, '']
                else
                    return [false, '']
    ...
    $('#rf_exclude_new_interval').click(function () {
        ...
        $('.tf_rf_exclude_interval:last').val('');
        $('.popbox').popbox();
        $('.tf_exclude_from_datepicker,.tf_exclude_to_datepicker').removeClass('hasDatepicker').removeAttr('id');
        $('.tf_exclude_from_datepicker,.tf_exclude_to_datepicker').datepicker({
            dateFormat:"yy-mm-dd",
            minDate: new Date(),
            beforeShowDay:function (date) {
                orientation_from = ($(this).hasClass('tf_exclude_from_datepicker')) ? true : false;
...
function generatePostData(form_object) {
    _form_data = {};
    _form_data['form_name'] = form_object.find('#tf_rf_formname_input').val();

    _form_data['submit_mess'] = form_object.find('#tf_rf_mess_submit').val();
    _form_data['succes_mess'] = form_object.find('#tf_rf_succ_mess').val();
    _form_data['fail_mess'] = form_object.find('#tf_rf_failure_mess').val();
    _form_data['label_in'] = form_object.find('#tf_rf_mess_datein').val();
    _form_data['label_out'] = form_object.find('#tf_rf_mess_dateout').val();
    _form_data['email_template'] = form_object.find('#tf_rf_email_template').val();
    _form_data['email_from'] = form_object.find('#tf_rf_email_from').val();
    _form_data['reservation_title'] = form_object.find('#tf_rf_res_title').val();
    _form_data['datepickers_count'] = form_object.find('#tf_rf_datepickers_count').val();
    _form_data['form_template'] = form_object.find('#tf_rf_form_template').val();
    if(form_object.find('#tf_rf_mess_reset').length > 0)
        _form_data['reset_button'] = form_object.find('#tf_rf_mess_reset').val();
    _form_data['new_res_email_template'] = form_object.find('#tf_rf_new_res_email_template').val();
    _form_data['email_subject'] = form_object.find('#tf_rf_email_subject').val();
    _form_data['required_text'] = form_object.find('#tf_rf_required_text').val();
    _form_data['header_message'] = form_object.find('#tf_rf_heading_text').val();
    _form_data['confirm_email_template'] = form_object.find('#tf_rf_confirm_email_template').val();
    _form_data['reject_email_template'] = form_object.find('#tf_rf_reject_email_template').val();
    form_date_inputs=form_object.find('#dates-to-exclude .tf_rf_exclude_interval');
    form_inputs = form_object.find('#form-content .tf_custom_reservationform_row');
    _count = 0;
    date_count=0;
    _form_data.types = {};
    _form_data.labels = {};
    ...
    _form_data.excludeDate = {};
    form_date_inputs.each(function(){
        _form_data.excludeDate[date_count]=$(this).val();
        date_count++;
    });
    form_inputs.each(function () {
        _form_data.types[_count] = $(this).find('.'+theme_index+'_inp_select:last').val();
        _form_data.labels[_count] = $(this).find('.rf_input_label:last').val();
        _form_data.width[_count] = $(this).find('.rf_input_width:last').val();
        _shortcode = $(this).find('.shortcode_code:last').html();
        _form_data.shortcode[_count] = _shortcode.substring(1, _shortcode.length - 1);
        _form_data.required[_count] = ($(this).find('label[for^=tf_rf_input_required]').hasClass('on')) ? 1 : 0;
        _form_data.newline[_count] = ($(this).find('label[for^=tf_rf_input_newline]').hasClass('on')) ? 1 : 0;
        if (_form_data.types[_count] == 2 || _form_data['types'][_count] == 4) {
            _options_count = 0;
            _form_data.options[_count] = {};
            _form_options = $(this).find('.tf_rf_input_options_label');
