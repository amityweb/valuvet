...
class TFUSE_WP_IMPORT extends TF_WP_Import {
    ...
    function tfuse_change_link(&$item, $key) {
        ...
        elseif ( !empty($key) && !empty($item) ) // remap url in widget_text, textareas options
        {
            $item = str_replace(array_keys($this->url_remap),$this->url_remap,$item);
        }
        elseif ( !empty($key) && !empty($item) && $key != 'upload_url' && strstr($item, $this->upload_url_old) )
        {
    ...
    function tfuse_import_options( $file ) {
        ...
        if ( !preg_match('|<tfuse_options>(.*?)</tfuse_options>|is', $file, $tfuse_options) )
            return;

        preg_match_all('|<item>(.*?)</item>|is', $tfuse_options[1], $imported_entries);

        foreach ($imported_entries[1] as $line) {
            preg_match('|<name>(.*?)</name>|is', $line, $name);
            preg_match('|<value>(.*?)</value>|is', $line, $value);
            $this->tfuse_options[$name[1]] = $value[1];
        }
        ...
        $upload_folder = end( explode('/', $this->upload_url_old) );
    ...
    function tfuse_process_options() {
        ...
        foreach ( $this->tfuse_options as $name => $value ) {
            ...
            $new_val = tfuse_unpk($value);
            if ( $new_val === false )
                $new_val = $value;

            if ( ($name == 'page_on_front' || $name == 'page_for_posts') && !empty($new_val) )
            ...
            if ( TF_THEME_PREFIX . '_tfuse_sidebars' == $name && !empty($new_val['settings']) )
            {
                ...
                {
                    $isposts = post_type_exists(end(explode('by_id_',$setting)));
...
//        }
    ...
    function backfill_attachment_urls() {
        ...
        foreach ( $rows as $row )
        {
            ...
            if ( is_array($new_val) )
                array_walk_recursive($new_val, array($this,'tfuse_change_link'));
            update_post_meta($row->post_id, TF_THEME_PREFIX.'_tfuse_post_options', $new_val);
