...
if (!defined('TFUSE'))
    wp_die(__('Direct access forbidden.'));
...
class TF_EXPORT extends TF_TFUSE {
    ...
    function make_export() {
        ...
        if ( isset( $_GET['download'] ) && ( isset( $_GET['page'] ) && $_GET['page'] == 'tf_export' ) )
        {
            ob_start();
...
//            die();
    ...
    function add_menu() {
        add_submenu_page('themefuse', 'Export', 'Export', 'manage_options', 'tf_export', array(&$this, 'export'));
    }
    
    function option_item( $name, $value ) {
        $this->item = '';
        if ( is_array($value) ) $value = tfuse_pk($value); 
