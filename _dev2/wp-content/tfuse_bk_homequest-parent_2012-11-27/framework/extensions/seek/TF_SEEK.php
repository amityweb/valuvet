...
class TF_SEEK extends TF_TFUSE {
    ...
    function __init() {
        ...
        if( $this->load->ext_file_exists($this->_the_class_name, '') ){
            // Extention exist
        } else {
            // Extentions not found in theme_config
            // do not load it
            return;
        }
        ...
        if (is_admin() && isset($_GET['page']) && in_array($_GET['page'], array('themefuse')))
            $this->add_static();
        else
            $this->add_static_clientside();

        $this->mem_custom_posts();
    ...
    public function init_action(){
        ...
        // Added in 'init' beacause admin options filter is fired too early and taxonomies are not defined
    ...
    private function add_static() {
        ...
        $this->include->css('seek', 'ext_seek_css');
    ...
    public function create_seek_property_post_type() {
        register_post_type($this->post_type, array(
            'labels'            => array(
                'name'          => $this->model->get_option('seek_property_name_plural', 'Seek Posts'),
                'singular_name' => $this->model->get_option('seek_property_name_singular', 'Seek Post'),
                'add_new_item'  => __('Add New ' . $this->model->get_option('seek_property_name_singular', 'Seek Post')),
    ...
    public function print_form_item($id){
        ...
        if( !in_array( $id, $form['items'] ) ){
            echo 'Udefined form item id: '.esc_attr($id).', in form id: '.esc_attr($form_id)."<br/>\n";
            return;
        }
        ...
        if( !isset( $items_options[$id] ) ){
            echo 'Udefined form item id: '.esc_attr($id)."<br/>\n";
            return;
        }
    ...
    private function build_search_where_sql($form_id){
        global $wpdb;
        ...
        if( !isset($forms_options[$form_id]) ){
            die('Undefined seek form option id: '.esc_attr($form_id));
        }
        ...
        if(sizeof($in_taxonomy_ids)){
            ...
            foreach($in_taxonomy_ids as $ids){
                ...
                foreach($regExpVals as $tid){
                    $regExpSql[] = $tid . $this->index_table_terms_separator;
                }
    ...
    public function do_search( $params = array() ) {
        ...
        if (empty($_GET[ TF_SEEK_HELPER::get_search_parameter('page') ])){
            $curr_page = 1;
        } else {
            $curr_page = (int) $_GET[ TF_SEEK_HELPER::get_search_parameter('page') ];
            if ($curr_page < 1)
                $curr_page = 1;
        }
        ...
        $form_id = @$_GET[ $this->get_search_parameter('form_id') ];
        if( !isset($forms_options[ $form_id ]) ){
            $form_id        = '';
            $built_where    = array(
                'sql'   => ''
            ...
            $built_where    = $this->build_search_where_sql($form_id);
        }

        $where_sql      = trim($built_where['sql']);
        $where          = " WHERE p.post_status = 'publish' AND p.post_type = '".$this->post_type."' " . ( $where_sql ? " AND ".$where_sql : "" );
        ...
        $sql = 'SELECT
            SQL_CALC_FOUND_ROWS *
                FROM ' . $this->db_table_name . ' AS options
        ...
        $total  = reset($wpdb->get_row('SELECT FOUND_ROWS() as total_rows', ARRAY_A));
    ...
    private function check_db() {
        ...
        $columns            = (
            sizeof( $wpdb->get_results( $wpdb->prepare("SHOW TABLES LIKE %s", $this->db_table_name) ) )
        ...
        foreach ($tables as $table) {
            $table          = (array) $table;
            $existing_tables= array_merge($existing_tables, array_values($table));
        }
        ...
        if (!in_array($this->db_table_name, $existing_tables)) {
        ...
        foreach ($options as $key => $val) {
            ...
            $type = preg_replace('/(\(.*\))/Uis', '', $existing_columns[$key]['Type']);
            if ($val['valtype'] != $type) {
                $must_recreate_table = TRUE;
                break;
            }
    ...
    private function create_index_table() {
        ...
        foreach ($options as $key => $data) {
            switch(@$data['valtype']){
                case 'int':
                    $str.="$key INT(255) NOT NULL DEFAULT  '" . intval($data['value']) . "',\n";
                break;
                default:
        ...
        $sql = "CREATE TABLE {$this->db_table_name} (
                    ...
                    _terms LONGTEXT CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
                    $str
    ...
    public function save_post_extra_processing($tfuse_post_options, $post_id) {
        ...
        // Taxonomy terms
        $_terms = '';
        if(sizeof($_POST['tax_input'])){
            $terms = array();
            foreach($_POST['tax_input'] as $tax=>$tax_vals){
                if(sizeof($tax_vals)){
        ...
        $tfuse_post_options['_terms'] = $terms;
    ...
    private function mem_custom_posts() {
        $this->mem->db['save_post_extension'] = array($this->post_type => &$this);
    }
    ...
    public function get_items_options(){
        return $this->get->ext_options($this->_the_class_name, 'items');
    }

    public function get_forms_options(){
        return $this->get->ext_options($this->_the_class_name, 'forms');
    }
    ...
    public function get_ext_setup_option($option) {
        ...
        if (!empty($cache)) {
            $cache = $this->get_ext_setup_options();
            if (isset($cache['tabs'])) {
                $tmp = tf_only_options($cache);
            }
        }
        ...
        } else if (isset($cache[$option]) || isset($tmp[$option])) {
            if (!isset($tmp)) {
                return $cache[$option];
            } else {
                return $tmp[$option];
    ...
    public function filter_seek_setup_options($options){
        ...
        foreach($property_options as $id=>$val){
            ...
            $opt_to_zones_options[] = array(
                ...
                'id'        => $option_zone_id,
                'value'     => ( ($tmp = $this->model->get_option($option_zone_id)) ? $tmp : $property_options[$id]['template_zone']),
                'type'      => 'select',
            ...
            $opt_to_zones_options[] = array(
                ...
                'value'     => ( ($tmp = $this->model->get_option($option_priority_id)) ? $tmp : $property_options[$id]['template_zone_priority']),
                'type'      => 'select',
        ...
        foreach($get_taxonomies as $taxonomy_name){
            ...
            $tax_to_zones_options[] = array(
                ...
                'id'        => $option_zone_id,
                'value'     => ( ($tmp = $this->model->get_option($option_zone_id)) ? $tmp : $default_value),
                'type'      => 'select',
                'options'   => $zones_select_options,
                'desc'      => __('Zone', 'tfuse'),
            );
            $tax_to_zones_options[] = array(
                'name'      => '&nbsp;',
                'id'        => $option_priority_id,
                'value'     => ( ($tmp = $this->model->get_option($option_priority_id)) ? $tmp : 0),
                'type'      => 'select',
                'options'   => $zones_select_priority_options,
                'divider'   => ($counter < $sizeof),
                'desc'      => __('Priority', 'tfuse'),
    ...
    private function get_template_zone_options($zone){
        ...
        if(isset($cache[$post->ID][$zone])){
            return $cache[$post->ID][$zone];
        }
