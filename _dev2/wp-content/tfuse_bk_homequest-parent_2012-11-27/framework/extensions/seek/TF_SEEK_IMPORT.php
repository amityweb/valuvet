...
class TF_SEEK_IMPORT {
    ...
    public function import_sql(){
        ...
        if(sizeof($tableExists)){
            ...
            $rowExists = $wpdb->get_results( "SELECT post_id FROM " . trim($wpdb->prepare("%s", TF_SEEK_HELPER::get_db_table_name()), "'") . " LIMIT 1", ARRAY_A);
    ...
    function action_tf_import_end( $args ){
        ...
        $table_name         = trim( $wpdb->prepare('%s', TF_SEEK_HELPER::get_db_table_name()), "'" );
