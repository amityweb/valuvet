...
class TF_SLIDER extends TF_TFUSE {
    ...
    function __init() {
        if (isset($_GET['page']) && $_GET['page'] == 'tf_slider' && isset($_GET['id'])) {
            $this->redirect_if_id_invalid($_GET['id']);
        }
        if (isset($_GET['page']) && ($_GET['page'] == 'tf_slider' || $_GET['page'] == 'tf_slider_list')) {
        ...
        if (is_admin() && isset($_GET['page']) && stripos($_GET['page'], 'tf_slider') === 0) {
            $this->add_static();
    ...
    public function options_extra_filter($val, $type) {
        if ($type == 'post') {
            ...
            if (count($extra_options) == 0)
                return $val;
            $val[] = array('name' => 'Post Fetched For Slider',
    ...
    protected function decide_design() {
        if (isset($_GET['id'])) {
            $slider = $this->model->get_slider($_GET['id']);
            $this->design = $slider['design'];
            $this->id = $_GET['id'];
            return;
        }
        if (!(isset($_GET['page']) && $_GET['page'] == 'tf_slider') || !(isset($_REQUEST['slider_design']) && in_array($_REQUEST['slider_design'], $this->valid_designs)))
            return;
        $this->design = strtolower($_REQUEST['slider_design']);
    }
    ...
    protected function decide_type() {
        if (isset($_GET['id'])) {
            ...
            $this->id = $_GET['id'];
        } elseif ((isset($_GET['page']) && $_GET['page'] == 'tf_slider') && (isset($_REQUEST['slider_type']) && in_array($_REQUEST['slider_type'], $this->valid_types))) {
            $this->type = $_REQUEST['slider_type'];
        }
        if (!empty($this->type))
    ...
    function add_menu() {
        if (count($this->general_options) <= 0)
            return;
        add_object_page('Sliders', 'Sliders', 'publish_pages', 'tf_slider_list', array(&$this, 'list_sliders'), '', 10);
        add_submenu_page('tf_slider_list', 'All sliders', 'All Sliders', 'publish_pages', 'tf_slider_list', array(&$this, 'list_sliders'));
        $design_from_page = str_replace('tf_slider_', '', tfget($_GET['page']));
        add_submenu_page('tf_slider_list', 'Add New Slider', 'Add New', 'publish_pages', 'tf_slider', array(&$this, 'add_new'));
...
#static part
    ...
    function add_static() {
        ...
        $this->include->js('slider', 'ext_slider_js', 'tf_footer');
    ...
    function tfuse_ajax_slider_save() {
        ...
        $slider_id = isset($_POST['slider_uniqid']) ? $_POST['slider_uniqid'] : '';
        $saved_options = json_decode(isset($_POST['options']) ? $_POST['options'] : '', TRUE);
        if ($this->model->title_exists($saved_options['general']['slider_title'], $slider_id))
            tfjecho(array('status' => -1, 'message' => 'The title you have specified is already in use with another slider. Please choose a different slider title.'));
        $id = $this->model->save_slides($slider_id, $saved_options, array('design' => $_POST['slider_design'], 'type' => $_POST['slider_type'])); //save_slides() returns ID of inserted row
    ...
    function tfuse_ajax_slider_title_exists() {
        ...
        if (isset($_POST['slider_title'])) {
            if ($this->model->title_exists($_POST['slider_title']))
                tfjecho(array('status' => -1, 'message' => 'A slider with the title you have provided, already exists. Please give a different title for your slider.'));
            else
                tfjecho(array('status' => 1, 'message' => 'OK'));
        }
        else
            tfjecho(array('status' => -1, 'message' => 'You have not provided a title for your slider. Please, do so.'));
        die();
    }
...
//wp_nonce_field( "{$args['id']}_tfuse_admin_meta_box", "{$args['id']}_tfuse_noncename" );
    ...
    function add_new() {
        if (!empty($_GET['id']) && $this->model->get_slider($_GET['id']) !== FALSE) {
            $this->add_slider();
            if (!empty($_GET['tab']) && $_GET['tab'] == 2)
                $this->include->js_enq('switch_to_tab', 1);
            return;
        }
        if ((isset($_POST['slider_design']) && in_array($_POST['slider_design'], $this->valid_designs)) && (isset($_POST['slider_type']) && in_array($_POST['slider_type'], $this->valid_types)) && !empty($_POST['slider_title'])) {
            $this->add_slider();
            $this->include->js_enq('slider_title_tmp', $_POST['slider_title']);
            $this->include->js_enq('switch_to_tab', 1);
        } else {
    ...
    function add_slider() {
        ...
        if (isset($_GET['delete']) && isset($_GET['id']) && $_GET['delete'] == 1) {
    ...
    public function get_sliders_dropdown( $types = array() ) {
        $sliders    = $this->model->get_sliders();
        $tmp        = array(-1 => __('Select Slider:', 'tfuse'));
