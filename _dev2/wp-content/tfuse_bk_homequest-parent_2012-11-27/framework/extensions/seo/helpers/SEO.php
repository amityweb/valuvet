...
function tfuse_seo_get_terms($id, $taxonomy) {
...
if (!function_exists('tfuse_mrt_get_url')) :

    function tfuse_mrt_get_url($query) {
...
function tfuse_get_paged($link) { // page poate fi nu doar page/2 dar &page=2 cind nu este setat permalinkul
...
if (!function_exists('tfuse_seo_replace_vars')):

    function tfuse_seo_replace_vars($text) {
        ...
        $replacements_complicate = array(
            '%%id%%' => $pa->ID,
            '%%tag%%' => tfuse_seo_get_terms($pa->ID, 'post_tag'),
            '%%title%%' => stripslashes($pa->post_title),
            '%%date%%' => $date,
            '%%category%%' => tfuse_seo_get_terms($pa->ID, 'category'),
...
if (!function_exists('tfuse_seo')) :
    ...
    function tfuse_seo() {
        ...
        if (tfuse_options('seo_use_canonical_url', false)) {
            global $wp_query;
            $url = tfuse_mrt_get_url($wp_query);
            if ($url) {
        ...
        if (is_home() || is_front_page() || is_tf_front_page()) {
            ...
            if (is_category()) {
                ...
                $title = tfuse_seo_replace_vars('%%date%%') . tfuse_options('seo_title_separator', ' | ') . __('Date Archive', 'tfuse') . tfuse_options('seo_title_separator', ' | ') . $theme_name;
                ...
                if (!$title) {
                    $title = single_term_title($term->name, false) . tfuse_options('seo_title_separator', ' | ') . $theme_name;
                    $allow_replacements = false;
                }
            ...
            $title = __('Search Results for ', 'tfuse') . sprintf(__('\'%s\''), $_GET['s']) . tfuse_options('seo_title_separator', ' | ') . $theme_name;
        ...
        if (is_paged()) {
            ...
            if ($paged_format) {
                $title .= ' ' . trim((string) tfuse_seo_replace_vars($paged_format));
            }
        }

        return array(
            'title' => ( $allow_replacements ? tfuse_seo_replace_vars($title) : $title ),
            'keywords' => $keywords,
...
function tfuse_add_meta() {
    ...
    echo '
    <meta name="description" content="' . htmlentities($tfuse_seo_meta['description'], ENT_QUOTES, "UTF-8") . '" />';

    if (tfuse_options('seo_use_meta_keywords', false) && trim($tfuse_seo_meta['keywords']))
        echo '
    <meta name="keywords" content="' . htmlentities($tfuse_seo_meta['keywords'], ENT_QUOTES, "UTF-8") . '" />';
    ...
    if ($tfuse_seo_meta['canonical'])
        echo '
    <link rel="canonical" href="' . htmlentities($tfuse_seo_meta['canonical'], ENT_QUOTES, "UTF-8") . '" />';
...
global $_tmp_bloginfo_rss_name;
ob_start();
    bloginfo_rss('name');
$_tmp_bloginfo_rss_name = ob_get_clean();

add_filter('wp_title_rss', 'tf_seo_wp_title_rss_filter', 11);
function tf_seo_wp_title_rss_filter($data){
    if( !tfuse_options('disable_tfuse_seo_tab') ){ // Change only if SEO is activated
        global $_tmp_bloginfo_rss_name;

        $exploded = explode($_tmp_bloginfo_rss_name, $data);
        if(!$exploded[0]){
            array_shift($exploded);
        }

        if(sizeof($exploded)>1){
            $data = implode(bloginfo_rss('name'), $exploded);
        } else {
...
function tfuse_add_title() {
    ...
    return htmlentities(trim($title), ENT_QUOTES, "UTF-8");
}

if (!is_admin() && !tfuse_options('disable_tfuse_seo_tab')) {
    add_filter('wp_title', 'tfuse_add_title');
    add_action('tfuse_meta', 'tfuse_add_meta');
}
...
class TF_SEO_SITEMAP {
    ...
    function BuildSitemap() {
        ...
        if (true) {
            ...
            $postCount = intval($wpdb->get_var( $wpdb->prepare("SELECT COUNT(*) AS cnt FROM `" . $wpdb->posts . "` WHERE " . $where ), 0, 0));
            ...
            if (true) {
                $postRes = mysql_query( $wpdb->prepare($sql), $wpdb->dbh);
                if (!$postRes) {
        ...
        if (true) {
            ...
            if (count($taxList) > 0) {
                ...
                $termInfo = $wpdb->get_results( $wpdb->prepare($sql) );
