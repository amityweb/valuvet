<div class="wrap">
    ...
    <h2><?php _e('ThemeFuse Updates'); ?></h2>

    <p>
        <?php printf(__('Last checked on %1$s at %2$s.'), date_i18n(get_option('date_format'), $updates->last_checked), date_i18n(get_option('time_format'), $updates->last_checked)); ?>
        &nbsp; <a class="button" href="<?php echo esc_url(self_admin_url('admin.php?page=tfupdates&action=checkagain')); ?>"><?php echo __('Check Again'); ?></a>
    </p>
    <?php
    if (!isset($response)) {
        ?>
        <h3>
            <?php printf(__('You have the latest version of %s.'), esc_html($this->theme->theme_name)); ?>
        </h3>

        <p>
            <?php printf(__('You have the latest version of %s. You do not need to update. However, if you want to re-install current version, you can do so automatically.'), esc_html($this->theme->theme_name)); ?>
        </p>

        <p>
            <?php printf(__('Upgrading %1$s will overwrite the current installed version of this theme. We recommend backing up your theme files before reinstalling the %1$s.'), esc_html($this->theme->theme_name)) ?>
        </p>
        ...
        <h3 class="response">
            <?php printf(__('An updated version of %s is available.'), esc_html($this->theme->theme_name)); ?>
        </h3>
    ...
    <p><?php echo __('While your site is being updated, it will be in maintenance mode. As soon as your updates are complete, your site will return to normal.') ?></p>
