...
jQuery(document).ready(function($) {
    ...
    $('#tfuse_admin_options_form').submit(function()
    {

        showLoading();
        var serializedValues = JSON.stringify($(this).serializeObject());
        var data = {
        ...
        $.post(ajaxurl, data, function(data)
        {
            ...
            showFinishedLoading();
            if(data !=null && data.reload_page==true) {
                setTimeout(function() {
...
function opt_set(optid,val) {
    ...
    if($('[name="'+optid+'"]').length>0) {
        $obj=$('[name="'+optid+'"]');
        if($obj.is('input[type="radio"]')) {
            $('[name="'+optid+'"]').filter('[value="'+val+'"]').prop("checked",true);
