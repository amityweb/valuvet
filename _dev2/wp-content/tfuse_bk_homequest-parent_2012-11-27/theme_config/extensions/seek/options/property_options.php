...
if( !function_exists('tf_seek_post_type_options__html_script_google_maps_input') ):
    ...
    <script type="text/javascript">
        jQuery(document).ready(function($){
            ...
            var map = null;
            var lat_element = $('input#<?php print $input_id_lat; ?>');
            var lng_element = $('input#<?php print $input_id_lng; ?>');

            var createSeekGMap = function(){
                ...
                var coods   = {
                    lat:    getFloatVal( lat_element.val() ),
                    lng:    getFloatVal( lng_element.val() )
                }
                ...
                map = new google.maps.Map(document.getElementById("<?php print $map_id; ?>"), myOptions);
                ...
                var placeMarker = function(position, noUpdateInputs) {
                    ...
                    if(!noUpdateInputs){
                        lat_element.val( String( coods.lat ) );
                        lng_element.val( String( coods.lng ) );
                    }
                ...
                lat_element.bind('blur change keyup', change_input);
                lng_element.bind('blur change keyup', change_input);
            };
            ...
            $('#seek_property_maps_has_position').bind('change', function(){
                ...
                if( $(this).is(':checked') ){
                    lat_element.removeAttr('disabled');
                    lng_element.removeAttr('disabled');
                    $('#<?php print $map_id; ?>').fadeIn(function(){
                        if(map===null && !firstTime){
                            createSeekGMap();
                        }
                    });
                    firstTime = false;
                } else {
                    lat_element.attr('disabled', 'disabled');
                    lng_element.attr('disabled', 'disabled');
                    $('#<?php print $map_id; ?>').fadeOut();
                }
            }).trigger('change');

            if(map===null) {
                if($('a[href=#tfusetab-google_maps]').length){
                    var stopInitMapsOnClick = false;
                    $('a[href=#tfusetab-google_maps]').click(function(){
                        if(stopInitMapsOnClick) return;
                        if(map===null && $('#seek_property_maps_has_position').is(':checked') ){
                            createSeekGMap();
                        }
                        stopInitMapsOnClick = true;
                    });
                } else {
                    if(map===null && $('#seek_property_maps_has_position').is(':checked') ){
                        createSeekGMap();
