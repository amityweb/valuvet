...
if ( ! function_exists( 'tfuse_header_content' ) ):
    ...
    {
        global $wp_query,$is_tf_front_page,$TFUSE,$post, $search, $header_element,$before_content_element, $latest_added_nr;
        $posts = $header_element = $header_image = $before_content_element = $slider = null;

        if (!$location) return;

        if (is_search() && $location == 'header')
        {
            get_template_part( 'header', 'search' );
            return;
        }
        elseif (is_search() && $location == 'before_content')
        {
            $jcarousel_sliders = $TFUSE->ext->slider->get_sliders_dropdown('jcarousel');
            $slider = array_search('FEATURED PROPERTIES', $jcarousel_sliders);
            if(!$slider) return;
        }
        elseif(is_tag() && $location == 'header')
        {
            $jcarousel_sliders = $TFUSE->ext->slider->get_sliders_dropdown('jcarousel');
            $slider = array_search('Featured Blog Posts', $jcarousel_sliders);
            if(!$slider) return;
        }
        else
        {
            switch ($location)
            {
                ...
                case 'before_content' :
                    if ($is_tf_front_page)
                    {
                        ...
                        {
                            $latest_added_nr = 8;
                            get_template_part( 'latest', 'added' );
                            return;
                        }
                        else
                        {
                    ...
                    elseif (is_tax())
                    {
                        ...
                        elseif('latest_added')
                        {
                            $latest_added_nr = tfuse_options('properties_number', -1 ,$ID);
                            get_template_part( 'latest', 'added' );
                            return;
                        }
                        else
                    ...
                    elseif ( is_singular() )
                    {
                        ...
                        elseif('latest_added')
                        {
                            $latest_added_nr = tfuse_page_options('properties_number', 8 ,$ID);
                            get_template_part( 'latest', 'added' );
                            return;
                        }
                        else
    ...
    function tfuse_get_slides_from_posts( $posts=array(), $slider = array() )
    {
        ...
        foreach ($posts as $k => $post) :
            ...
            elseif ($post->post_type == TF_SEEK_HELPER::get_post_type())
            {
                ...
                {
                    $temp_title = tfuse_page_options('title_for_slider', '', $post->ID);
                    $single_image = tfuse_page_options('thumbnail_image', get_template_directory_uri() . '/images/dafault_image.jpg', $post->ID);
                }
